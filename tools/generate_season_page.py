...
def build():
    ...
    out.append({
        ...
        "text": [
            ...
            "- **钥匙命名**：游戏官方串表里 `骷髅钥匙` 是**可重复使用**的版本（`rkey`，Skeleton Key Unlimited）；"
            "世界掉落的那把消耗品钥匙（`key`，Skeleton Key）官方串表没有条目，本 wiki 按说明书的叫法记作 **普通钥匙**。"
            "说明原文「珍稀宝物」一节写的『骷髅钥匙』指的是后者（世界掉落的那把）。",
