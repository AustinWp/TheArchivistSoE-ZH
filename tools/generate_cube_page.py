...
def main():
    ...
    mats = [
        ...
        ["神话宝珠 + 魔法大型护符", "基得的运气（固定物品）"],
        ["神话宝珠 + 魔法/稀有戒指", "随机低阶暗金戒指（拿各的戒指 / 玛那得的治疗 / 乌鸦之霜 / 矮人之星 / 腐肉之风）"],
        ["神话宝珠 + 魔法/稀有项链", "随机低阶暗金项链（诺科蓝遗物 / 猫眼 / 玛哈姆橡木古董 / 萨拉森的机会 / 新月 / 艾利屈之眼 / 亚特玛的圣甲虫）"],
    ...
    sec["text"] += [
        ...
        *md_table(["基底", "成功", "失败（物品被摧毁 → 1×普通钥匙）"], [
        ...
        "- **低成本路线（底材不用刷）**：`商店买普通级基底` → {{icon:ooal}}炼金宝珠点金（白色/魔法 → 稀有）→ "
        "**稀有物品升阶**两次（普通 → 扩展 → 精英，配方见「物品升级 / 修理 / 分解」节）→ 机遇宝珠判定。"
        "例：{{icon:aar}}古代装甲 → {{icon:xar}}华丽战甲 → {{icon:uar}}神圣盔甲，终点是该基底上的"
        "**泰瑞尔的力量** / **圣堂武士的力量**。失败会摧毁底材，所以每次判定都要重走一遍："
        "1×{{icon:ooal}}炼金宝珠 + 两套符文与完美宝石 + 1 枚机遇宝珠（= 10 枚机遇碎片）。"
        "标准模式同样可以这样攒底材，只是精英基底最后改用{{icon:dvo}}神授宝珠",
        "- **为什么必须先点金**：源码里有升阶公式的只有**稀有（`rar`）/ 套装 / 暗金**物品"
        "（如 `armo,bas,rar → armo,exc,rar`、`armo,exc,rar → armo,eli,rar`），**白色与魔法物品没有升阶行** —— "
        "所以「先点成稀有」是这条路线成立的前提，不是可省的一步",
        "- **底材等级**：本组配方行的 `ilvl` 列为空、输出 `lvl` 固定 `99`，"
        "即**源码层面没有给输入底材设物品等级门槛** —— 商店买的底材同样可用（游戏内实测未验证）",
        "- ⚠️「底材能在商店买到」属游戏内事实：本站参考数据只有官方串表 + 两张 `CubeMain`，"
        "**没有商店表可以复核**，该句按社区经验收录",
    ...
    add("mythic-orb-recipes", "神话宝珠配方（仅标准模式）", [
        ...
        "- **获取途径（标准模式）**：各幕 `Act X Good` 掉落池 → `New orbs` 子池（普通/噩梦/地狱与 T1–T3 地图、"
        "`Dungeon Orbs` 地下城、`Terror currency` 之怖、超级古代人、卢西恩宝箱、`Treasure Fallen H`），"
        "以及炼狱熔炉「烬魂印钞」（约 5%）",
        *md_table(["材料", "结果"], mats),
    ], modes=["standard"])
    add("divine-orb-recipes", "神授宝珠配方", [
        "- ⛔ **炼狱（毁灭）模式拿不到神授宝珠**：掉落表 0 处、炼狱熔炉印钞不产出、也没有产出它的配方"
        "（唯一的 `旧神授宝珠 → 神授宝珠` 转换配方，其输入「旧神授宝珠」同样已无任何来源）。"
        "但该模式仍有 **31 条「指定地狱锻铸暗金」+ 1 条「末日之刃复制品」**需要它 —— "
        "此处按代码如实标注，游戏内是否另有发放途径（服务端）请以实测为准",
        "- ℹ️ 注意：**狱铸（地狱锻铸）暗金本身在炼狱模式仍会正常掉落**（源码 `enabled=1`、`rarity` 1~5），"
        "所以这不影响「能不能拿到狱铸装备」，只影响「能不能**指定**要哪一件」",
        "- **获取途径（标准模式）**：T1–T3 地图 `New orbs T1/T2/T3`、`New orbs fallen`、"
        "`Dungeon Orbs` 地下城、超级古代人、卢西恩宝箱，以及炼狱熔炉「烬魂印钞」（约 3.5%）",
        "- 结果物品必定为**有形**，即使输入物品为无形",
        *md_table(["材料", "结果"], [
            ...
            ["神授宝珠 + 魔法/稀有戒指", "随机高阶暗金戒指（乔丹之石 / 布尔凯索之戒 / 大自然的和平 / 收缩之环 / 鬼火投射者 / 死亡的平衡 / 兄弟会的召唤 / 巴尔的通行证 / 巴尔的喘息 / 巴尔之握）"],
            ["神授宝珠 + 魔法/稀有项链", "随机高阶暗金项链（旭日东升 / 大君之怒 / 马拉的万花筒 / 炽天使之韵 / 金属网格 / 命运的抗争 / 阿斯特拉门蒂斯）"],
        ]),
        "- 炼狱（毁灭）模式中，神授宝珠**仅用于**炼狱熔炉的「指定地狱锻铸暗金」与「末日之刃复制品」，"
    ...
    add("rune-downgrading-high", "高位符文降级（仅标准模式）", [
        "- 炼狱（毁灭）模式**没有**古尔及以上高符文的降级配方",
        *md_table(["符文", "附加材料", "结果"], downgrade_table(high)),
    ], modes=["standard"])
    ...
    # ------------------------------------------------------------------ 6 恶魔宝盒
    add("demonic-cube", "恶魔宝盒（重洗数值）", [
        "- 恶魔宝盒可重洗合格暗金/套装的可变数值（不改变底材、孔数与孔中内容）",
        "- 基得的运气 / 地狱火炬 / 毁灭小护符 均可重洗",
    ...
    # 标准模式专属：珠宝/护符「类型重掷」（炼狱模式由碎片体系取代）
    add("demonic-cube-reroll", "恶魔宝盒 · 珠宝/护符类型重掷（仅标准模式）", [
        "- 炼狱（毁灭）模式**不存在**这组「类型重掷」配方，改由碎片体系取代（见「炼狱模式专属」）",
        *md_table(["材料", "结果"], [
            ["普通暗金珠宝 + 永恒币 ×1 + 恶魔宝盒 ×1", "重掷为另一枚普通暗金珠宝"],
            ["暗金神话珠宝 + 永恒币 ×1 + 恶魔宝盒 ×5", "重掷为另一枚暗金神话珠宝"],
            ["暗金华丽护符 + 永恒币 ×1 + 恶魔宝盒 ×5", "重掷为另一枚暗金华丽护符"],
            ["未腐化普通/神话/暗金珠宝 + 永恒币 + 恶魔宝盒", "重掷类型（按珠宝类型消耗不同数量的宝盒）"],
        ]),
    ], modes=["standard"])

    # ------------------------------------------------- 6.5 驯服（The Taming）
    add("the-taming", "驯服（The Taming）", [
        "- 用**三枚巴尔戒指**合成：巴尔通行证 + 巴尔喘息 + 巴尔之握 → **驯服**",
        "- 「驯服」**不掉落**（源码 `rarity=0`），只能这样合成；两种模式都能合成",
        "- 属性：获得相当于对应**未封顶元素抗性 5%** 的该元素穿透、+10-15 所有抗性、+1 所有技能",
        "- 官方串表：`The Taming`（名称）、`StrTamingAura`（穿透说明）、`StrTamingHint`（背景语）",
    ])
    ...
    add("alchemy-chaos-eternal", "炼金宝珠 / 混沌宝珠 / 永恒宝珠", [
        *md_table(["材料", "结果"], [
        ...
        "- 注：`永恒宝珠` / `永恒宝珠印记` 在源码里是**每个基底各写一行**"
        "（武器 296 行、护甲 220 行，等于全部基底），所以看起来像「逐件点名」，实际覆盖全部底材",
    ])
    ...
    blk = [[fmt_out(r), _pct] for r, _pct in zip(clust, ["45%", "35%", "15%", "4%", "1%"])]
    ...
    add("infernal-kiln", "炼狱熔炉", [
        ...
        *md_table(["配方", "结果"], [
            ...
            [f"{kiln_cost_txt('FATE CARD GAMBA - ROLL')} + 任意命运卡", "无变化 / 消失 / 单独复制一张（三者之一，概率以游戏内为准；放入方块判定，一次出结果）"],
    ...
    # ------------------------------------------------------------------ 20 升级/修理/分解
    add("socketing-standard", "打孔 / 重铸 · 拉苏克谜盒合成（仅标准模式）", [
        "- 炼狱（毁灭）模式**没有**这条配方",
        *md_table(["材料", "结果"], [
            ["2×拉苏克谜盒碎片 + 1×永恒币", "拉苏克谜盒"],
        ]),
    ], modes=["standard"])
