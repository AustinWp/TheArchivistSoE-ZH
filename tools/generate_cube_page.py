...
def main():
    ...
    add("about", "关于魔方配方", [
        ...
        "- 部分条目在官方数据中查无配方行（`重塑宝珠`、`意念宝珠`、`恰西的工艺箱` 等），本站按【官方数据校验】呈现并注明；详见「疑点与待验证」",
    ])
    ...
    add("demonic-cube", "恶魔宝盒（重洗与重掷）", [
        ...
        *md_table(["材料", "结果"], [
        ...
        "- ⚠️ 旧资料记载「恶魔宝盒可移除圣化/亵渎状态」，当前数据中**未找到对应配方行**，请在游戏内确认后再使用（见「疑点与待验证」）",
    ...
    add("hellfire-torch-desecration", "地狱火炬亵渎", [
        ...
        *md_table(["亵渎词缀"], [[x] for x in torch_names if x != "Poof to ashes!"]),
        "- 符合条件且未腐化时，恶魔宝盒可移除亵渎状态（⚠️ 当前数据未见对应行，待游戏内确认）",
    ])
    ...
    add("crafting-infusions", "制作灌注物（官方名：×系模组）", [
        ...
        *md_table(["灌注物", "符文", "完美宝石", "其他"], inf_rows),
        ...
        "- 恰西的工艺箱：单独放进方块并连续合成两次 → 随机 5 份同类灌注物（官方说明串提到「或催化剂碎片」，当前数据仅 7 种灌注物，待实测）",
    ])
    ...
    add("chisels", "制图师凿子", [
        ...
        "- `制图师凿子·增殖` 官方名称存在，但当前数据**无对应配方行**（占位，暂不收录）",
    ...
    # ------------------------------------------------------------------ 23 疑点
    add("pending-verification", "疑点与数据校验说明", [
        "- 本站已对两模式实际物品表（`misc.bin`）做字符串验证，以下条目**物品与配方均未上线**，不收录为正式配方：",
        "  - `重塑宝珠`（reso）、`意念宝珠`（ooin）：串表有功能描述，物品表/配方表均不存在 → 未上线",
        "  - `基德的珍奇箱`（gcbx）：串表有名称，物品表/配方表均不存在 → 未上线",
        "  - `恰西的工艺箱`（ccbx）：**配方行存在**（7 种灌注物结果、结果池无催化剂碎片），但物品未进入物品表 → 当前版本无法获得，疑似未上线/活动物品；若游戏内可获取请反馈",
        "- **恶魔宝盒移除圣化/亵渎状态**：配方文本表无对应行、串表无描述 → 疑似旧版残留；如游戏内实际可行请反馈，将立即修正文案",
        "- **制图师凿子·增殖 / 恶魔宝盒(已充能)**：物品存在于物品表，但无配方行 → 占位，未收录",
        "- 官方串表自身矛盾两处：圣者宝藏卡片说明（3→2×）与配方（2→3×）、恰西工艺箱说明（含催化碎片）与配方（无）——本站以**配方数据**为准",
        f"- 数据基准：SOECN 分支 commit `374d8971`（检查日期 2026-08-26）",
    ])
