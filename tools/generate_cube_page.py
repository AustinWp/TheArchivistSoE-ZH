...
def main():
    ...
    # ------------------------------------------------------------------ 23 疑点
    add("pending-verification", "疑点与待验证", [
        "- **恰西的工艺箱**：官方说明串写「5 份随机工艺注能**或催化剂碎片**」，当前数据结果池只有 7 种灌注物 → 待实测",
        "- **基德的珍奇箱 / 制图师箱子**：官方名称存在，当前数据无配方行 → 待实测",
        "- **恶魔宝盒移除圣化/亵渎状态**：当前数据无对应行 → 待实测",
        "- **重塑宝珠 / 意念宝珠**：官方串表有功能描述，但无物品、无配方行 → 判定为占位/未上线，不收录为正式配方",
        "- **制图师凿子·增殖**：官方名称存在，无配方行（占位）",
        "- **恶魔宝盒(已充能)**：物品存在（Imbue Random Ready），无配方行（占位）",
