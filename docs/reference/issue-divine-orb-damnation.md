...
不掉落、印钞不产出、也没有产出它的配方。这 32 条配方目前等于**死配方**。
...
- 炼狱模式下玩家**无法完成**这 32 条配方，尤其「指定地狱锻铸暗金」是**获取地狱锻铸装备的唯一路径**
  （随机狱铸配方已在本版本被禁用，见 `enabled 1→0`）。
- 若服务端另有发放方式（例如管理发放、活动奖励、或某张表不在本仓库内），请忽略本报告；
  但**仅就本仓库的数据而言，这条路线是断的**。
