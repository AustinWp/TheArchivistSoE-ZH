...
### P2 · 视觉换风格（改动面大，需先定方向）
衬线中文标题、暖黑底 + 金色卡片、卡片化列表、顶栏品牌区。插画 banner 若要用图，需要美术素材（可用纯 CSS 渐变/纹理先替代）。
...
- **插画 banner**：`s1-druid-*.webp` 是国服自己的赛季插画，是否可用需确认。
- **图片版权**：物品贴图源自游戏客户端 `pd2data.mpq`，与本站记录的是同一款游戏；建议在关于页写明来源并致谢。
