...
CubeMain 里带 `ROLL`(op18) 行的机制都是**两段式**：第一次掷骰登记（输出 `useitem`，物品原样退回），
第二次才按 `OUTCOME`(op16) 的累计阈值结算。官方串表也明写「合成两次」
（`hfmx` 烬魂簇 / `dsno` 亵渎宝珠 / `umbx` 炼狱熔炉箱 / `cabx` 制图师之箱 / `ccbx` 恰西的工艺箱 / `gcbx` 基德的珍奇箱）。

涉及：机遇宝珠、崇高宝珠、烬魂簇、仇恨宝珠重洗、命运卡增殖、地狱火炬亵渎、炼狱熔炉印钞。
**判断方法**：该机制在 `CubeMain` 里有没有 `- ROLL` 段（`op=18`）。有 → 两次；没有（如各类 OUTCOME-only 的池子）→ 一次。
